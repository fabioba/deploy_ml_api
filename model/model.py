...
def train_model(df_train):
    ...
    try:
        ...
        classifier = smf.glm('salary ~  capital_loss + age + hours_per_week + fnlgt + education_num + capital_gain + C(education) + C(marital_status) + C(native_country) + C(occupation) + C(race) + C(relationship) + C(sex) + C(workclass)', family=sm.families.Binomial(), data=df_train).fit(maxiter=1000)
